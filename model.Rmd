...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, labels = FALSE)
```
...
library(arulesCBA)
library(tidyverse)
resumes <- read.csv("resume_data.csv")
```
...
resumes_short <- resumes %>%
  ...
  filter(X.job_position_name == 
           c("Senior Software Engineer", "Machine Learning (ML) Engineer", 
             "Senior iOS Engineer", "AI Engineer", "Database Administrator (DBA)", 
             "Network Support Engineer", "DevOps Engineer", "Data Engineer", 
             "Full Stack Developer (Python,React js)", "Data Science Engineer"))


resumes_short$score_bin <- discretize(resumes_short$matched_score,method="interval",breaks=10)
```
...
rules <- apriori(
  trans,
  parameter = list(sup = 0.01, conf = 0.60, minlen = 2),
